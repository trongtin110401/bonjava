...
killProcess() {
  ...
  kill -9 $(ps aux | grep "vbee-1.0.jar" | grep -v 'grep' | awk '{print $2}')
  kill -9 $(ps aux | grep "wsreport-1.0-SNAPSHOT.jar" | grep -v 'grep' | awk '{print $2}')
  kill -9 $(ps aux | grep "VinPlayBackend-1.0.jar" | grep -v 'grep' | awk '{print $2}')
