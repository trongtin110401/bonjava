...
SELECT *
FROM vinplay_minigame.thanh_du;
...
DELIMITER;;
CREATE
DEFINER=`root`@`%` PROCEDURE `update_pot_md5`(IN pot_name VARCHAR(45),
    IN new_value BIGINT)
BEGIN
UPDATE minigame_pots
SET `value` = new_value
WHERE minigame_pots.`pot_name` = pot_name;
END ;;
DELIMITER ;
...
DELIMITER;;
CREATE
DEFINER=`root`@`%` PROCEDURE `update_fund`(IN fund_name VARCHAR(45),
    IN new_value BIGINT)
BEGIN
...
BEGIN
  SET
@min_id_today = (SELECT `value` FROM key_value WHERE updated_at = CURDATE() AND `key` = CONCAT('taixiu_', money_type));
    IF
...
SET `value`    = (SELECT min(id)
    ...
    updated_at = CURDATE()
WHERE `key` = CONCAT('taixiu_', money_type);
SET
@min_id_today = (SELECT `value` FROM key_value WHERE updated_at = CURDATE() AND `key` = CONCAT('taixiu_', money_type));
END IF;
...
BEGIN
SELECT *
FROM tx_get_chi_tiet_phien_md5
WHERE transaction_detail_tai_xiu.reference_id = reference_id
  AND transaction_detail_tai_xiu.money_type = money_type
