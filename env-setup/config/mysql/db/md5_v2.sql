...
(
    ...
    `dice1`            tinyint(4) DEFAULT NULL,
    `dice2`            tinyint(4) DEFAULT NULL,
    `dice3`            tinyint(4) DEFAULT NULL,
    `total_tai`        bigint(20) DEFAULT NULL,
    `total_xiu`        bigint(20) DEFAULT NULL,
    `num_bet_tai`      int(11) DEFAULT NULL,
    `num_bet_xiu`      int(11) DEFAULT NULL,
    `total_prize`      bigint(20) DEFAULT NULL,
    `total_refund_tai` bigint(20) DEFAULT NULL,
    `total_refund_xiu` bigint(20) DEFAULT NULL,
    `total_revenue`    bigint(20) DEFAULT NULL,
    `money_type`       tinyint(4) DEFAULT NULL,
    `timestamp`        timestamp NULL DEFAULT CURRENT_TIMESTAMP,
    `total_exchange`   mediumtext COLLATE utf8_unicode_ci,
...
) ENGINE=InnoDB AUTO_INCREMENT=12221 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci ROW_FORMAT=DYNAMIC;
...
$$
CREATE
DEFINER=`root`@`%` PROCEDURE `save_result_tai_xiu_md5`(IN reference_id BIGINT,
IN result INT(11),
IN dice1 TINYINT(4),
IN dice2 TINYINT(4),
IN dice3 TINYINT(4),
...
BEGIN
INSERT INTO result_tai_xiu_md5 (`reference_id`, `result`, `dice1`, `dice2`, `dice3`, `total_tai`, `total_xiu`,
                                `num_bet_tai`, `num_bet_xiu`, `total_prize`, `total_refund_tai`, `total_refund_xiu`,
                                `total_revenue`, `money_type`)
VALUES (reference_id, result, dice1, dice2, dice3, total_tai, total_xiu, num_bet_tai, num_bet_xiu, total_prize,
        total_refund_tai, total_refund_xiu, total_revenue, money_type);
END$$
DELIMITER ;
...
INSERT INTO `transaction_detail_tai_xiu_md5` (`reference_id`, `transaction_code`, `user_id`, `user_name`, `bet_value`,
...
$$
CREATE
DEFINER=`root`@`%` PROCEDURE `save_transaction_tai_xiu_md5`(IN reference_id BIGINT,
IN user_id INT(11),
IN user_name VARCHAR(45),
IN bet_value BIGINT,
IN bet_side TINYINT(4),
...
$$
CREATE
DEFINER=`root`@`%` PROCEDURE `tx_count_lich_su_giao_dich_md5`(IN nickname VARCHAR(45),
...
$$
CREATE
DEFINER=`root`@`%` PROCEDURE `tx_get_chi_tiet_phien_md5`(IN reference_id BIGINT,
    IN money_type TINYINT(4))
BEGIN
SELECT *
FROM transaction_detail_tai_xiu_md5
WHERE transaction_detail_tai_xiu_md5.reference_id = reference_id
  AND transaction_detail_tai_xiu_md5.money_type = money_type
...
$$
CREATE
DEFINER=`root`@`%` PROCEDURE `tx_get_lich_su_giao_dich_md5`(IN nickname VARCHAR(45),
    IN page_number INT,
    IN money_type TINYINT(4))
BEGIN
  declare
...
num_end int;
    set
num_start = (page_number - 1) * 10;
    set
num_end = page_number * 10;
SELECT tran_tx.reference_id,
       tran_tx.user_id,
       tran_tx.user_name,
       tran_tx.bet_value,
       tran_tx.bet_side,
       tran_tx.total_prize,
       tran_tx.total_refund,
       tran_tx.`timestamp`,
       result_tx.dice1,
       result_tx.dice2,
       result_tx.dice3,
       tran_tx.total_exchange
FROM (SELECT *
      FROM transaction_tai_xiu_md5
      WHERE user_name = nickname
        AND transaction_tai_xiu.money_type = money_type
...
$$
CREATE
DEFINER=`root`@`%` PROCEDURE `tx_get_top_thanh_du_md5`(IN start_time VARCHAR(45),
    IN end_time VARCHAR(45),
    IN in_type TINYINT)
BEGIN
SELECT *
FROM thanh_du
WHERE start_time <= thanh_du.last_update
  AND thanh_du.last_update <= end_time
  AND thanh_du.`type` = in_type
ORDER BY thanh_du.`number` DESC, thanh_du.total_betting DESC, thanh_du.last_update ASC LIMIT 0,10;
END$$
DELIMITER ;
...
$$
CREATE
DEFINER=`root`@`%` PROCEDURE `tx_update_thanh_du_md5`(IN user_name VARCHAR(45),
    IN in_number INT,
...
SELECT 1 = 1
FROM thanh_du_md5
WHERE thanh_du_md5.user_name = user_name
  AND thanh_du_md5.type = in_type
  AND thanh_du_md5.`last_update` >= CURDATE()) THEN
BEGIN
UPDATE thanh_du_md5
SET thanh_du_md5.`number`       = in_number,
    thanh_du.total_betting      = total_betting,
    thanh_du_md5.last_reference = current_reference_id,
    thanh_du_md5.`references`   = in_references
WHERE thanh_du_md5.user_name = user_name
  AND thanh_du_md5.type = in_type
  AND thanh_du_md5.`last_update` >= CURDATE();
END;
ELSE
BEGIN
...
BEGIN
  SET
@min_id_today = (SELECT `value` FROM key_value WHERE updated_at = CURDATE() AND `key` = CONCAT('taixiu_md5_', money_type));
    IF
ISNULL(@min_id_today) THEN
UPDATE key_value
SET `value`    = (SELECT min(id)
                  FROM transaction_tai_xiu_md5
                  WHERE `timestamp` >= CURDATE()
                    AND transaction_tai_xiu_md5.money_type = money_type),
    updated_at = CURDATE()
WHERE `key` = CONCAT('taixiu_md5_', money_type);
SET
@min_id_today = (SELECT `value` FROM key_value WHERE updated_at = CURDATE() AND `key` = CONCAT('taixiu_md5_', money_type));
END IF;
...
$$
CREATE
DEFINER=`root`@`%` PROCEDURE `update_fund_md5`(IN fund_name VARCHAR(45),
    IN new_value BIGINT)
BEGIN
UPDATE minigame_funds
SET minigame_funds.`value` = new_value
WHERE minigame_funds.`fund_name` = fund_name;
END$$
DELIMITER ;
...
$$
CREATE
DEFINER=`root`@`%` PROCEDURE `update_luot_rut_loc_md5`(IN user_name VARCHAR(45),
    IN luot_rut INT)
BEGIN
INSERT INTO user_rut_loc_md5(`user_name`, `so_lan_rut`)
VALUES (user_name, luot_rut) ON DUPLICATE KEY
...
$$
CREATE
DEFINER=`root`@`%` PROCEDURE `update_transaction_tai_xiu_detail_md5`(IN in_transaction_code VARCHAR(45),
    IN prize BIGINT,
    IN refund BIGINT)
BEGIN
UPDATE transaction_detail_tai_xiu_md5
SET `prize`  = prize,
    `refund` = refund
